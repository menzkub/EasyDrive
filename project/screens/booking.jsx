// Booking screen — form + Leaflet map for destination pin

function BookingScreen({ user, vehicles, bookings, users = [], onSubmit, prefillVehicle, onCancel }) {
  const [step, setStep] = React.useState(1); // 1: details, 2: vehicle, 3: confirm
  const [form, setForm] = React.useState({
    purpose: window.PURPOSES[0],
    purposeNote: "",
  ...
  // Available vehicles - no conflict
  ...
  return (
    <div style={{maxWidth:1200, margin:'0 auto'}}>
      ...
      <div style={{display:'flex', gap:10, marginTop:18, justifyContent:'flex-end'}}>
        {step > 1 && <button className="btn ghost" onClick={() => setStep(step - 1)}>{window.I.arrowLeft} ย้อนกลับ</button>}
        <button className="btn" onClick={onCancel}>ยกเลิก</button>
        {step < 3 && <button className="btn primary" onClick={next}>ถัดไป {window.I.arrowRight}</button>}
        ...
        <div className="col gap-3">
          <div className="field">
            ...
            <select className="select" value={form.purpose} onChange={(e) => setForm({...form, purpose:e.target.value})}>
              {window.PURPOSES.map((p) => <option key={p}>{p}</option>)}
            </select>
          </div>
          <div className="field">
        ...
        <div style={{padding:'16px 18px 12px'}}>
          ...
          <div style={{display:'flex', gap:6, flexWrap:'wrap'}}>
            {presetLocations.map((p) => (
              <button key={p.name} className="btn sm ghost" onClick={() => setLocation(p)}>{window.I.pin} {p.name}</button>
            ))}
          </div>
        </div>
...
function Step2({ form, setForm, vehicles, availableVehicles, conflictVehicles, bookings, users }) {
  ...
  // Compute conflict info per vehicle for the chosen time range
  const conflictMap = {};
  vehicles.forEach((v) => {
  ...
  // Show ALL bookable vehicles (skip maintenance/unavailable)
  ...
  return (
    <div className="card card-pad">
      <div style={{display:'flex', alignItems:'baseline', gap:14, flexWrap:'wrap'}}>
        <div>
          ...
          <p className="sub" style={{margin:'2px 0 0'}}>
            ในช่วง {window.fmtDateTime(form.from)} ถึง {window.fmtDateTime(form.to)} —
        ...
        <div style={{display:'flex', gap:6, marginLeft:'auto', flexWrap:'wrap'}}>
          {types.map((t) => (
            <button key={t} className={"btn sm" + (typeFilter === t ? " primary" : " ghost")} onClick={() => setTypeFilter(t)}>
              {t === "all" ? "ทั้งหมด" : window.VEHICLE_TYPES[t]?.label || t}
            </button>
          ))}
        </div>
      </div>
                  ...
                  <div key={b.id} style={{
                    ...
                    <div style={{flex:1, minWidth:0}}>
                      ...
                      <div className="text-xs muted" style={{whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>
                        {window.fmtDateTime(b.from)}–{window.fmtTime(b.to)} · {b.destination}
                      </div>
                    </div>
                  </div>
                );
          ...
          return (
            <button key={v.id} onClick={() => setForm({...form, vehicleId: v.id})}
              ...
              <div className="veh-ico lg" style={{
                ...
                <window.VehicleIcon type={v.type} size={36}/>
              </div>
              <div style={{flex:1, minWidth:0}}>
                ...
                <div className="text-xs muted">{window.VEHICLE_TYPES[v.type]?.label} · {v.seats} ที่นั่ง · {window.FUEL_TYPES[v.fuel]}</div>
              </div>
              {selected && (
                <div style={{
      ...
      {/* Maintenance vehicles displayed separately */}
    ...
    <div className="grid-2" style={{gap:18, gridTemplateColumns:'1.3fr 1fr'}}>
      <div className="card card-pad">
        ...
        <div style={{display:'grid', gridTemplateColumns:'140px 1fr', rowGap:10, columnGap:14, marginTop:14, fontSize:13.5}}>
          ...
          <Field label="วัน-เวลา ไป" value={window.fmtDateTime(form.from)}/>
          <Field label="วัน-เวลา กลับ" value={window.fmtDateTime(form.to)}/>
        ...
        <div className="card card-pad">
          ...
          <div style={{display:'flex', gap:14, alignItems:'center', padding:'14px 0', borderBottom:'1px solid var(--border)'}}>
            <div className="veh-ico lg" style={{width:64, height:64, background:'var(--pea-purple-50)', color:'var(--pea-purple)'}}>
              <window.VehicleIcon type={v.type} size={44}/>
            </div>
            <div style={{flex:1}}>
              ...
              <div className="text-xs muted">{v.id} · {v.year} · {window.VEHICLE_TYPES[v.type]?.label}</div>
            </div>
          </div>
                ...
                {conflicts.map((b) => {
                  ...
                  return (
                    <div key={b.id} style={{background:'white', borderRadius:6, padding:'6px 10px', fontSize:11.5, border:'1px solid #f0d8a8', display:'flex', gap:6, alignItems:'center'}}>
                      ...
                      <span className="muted" style={{flex:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>· {window.fmtTime(b.from)}-{window.fmtTime(b.to)} · {b.destination}</span>
                    </div>
                  );
                })}
              </div>
