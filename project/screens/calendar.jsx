...
function DayDetailModal({ date, events, vehicles, users, onClose, onSelectBooking }) {
  ...
  return (
    <window.Modal title={`การจองรถ · ${dateLabel}`} onClose={onClose} width={720}>
      {events.length === 0 ? (
              ...
              return (
                ...
                onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                  <div style={{
                    ...
                    <div style={{fontSize:13, fontWeight:700, fontFamily:'var(--font-mono)'}}>{window.fmtTime(b.from)}</div>
                    <div style={{fontSize:10, color:'var(--text-3)'}}>ถึง</div>
                    <div style={{fontSize:13, fontWeight:700, fontFamily:'var(--font-mono)'}}>{window.fmtTime(b.to)}</div>
                  </div>
                  <div className="veh-ico" style={{background:'var(--surface-2)'}}>
                    {v && <window.VehicleIcon type={v.type} size={24}/>}
                  </div>
                  ...
                  <div style={{color:'var(--text-3)'}}>{window.I.arrowRight}</div>
                </div>
              );
            })}
          </div>
              ...
              {evts.slice(0, 3).map((b) => {
                ...
                return (
                  <div key={b.id} className={"cal-event " + b.status}
                    ...
                    {window.fmtTime(b.from)} {v?.plate.split(' ')[1] || ''} · {u?.name.split(' ')[0]}
                  </div>
                );
              })}
              {evts.length > 3 && (
                      ...
                      {evts.map((b) => {
                        ...
                        return (
                          <div key={b.id} className={"cal-event " + b.status} onClick={() => onSelectBooking(b)} style={{fontSize:10.5, padding:'3px 6px'}}>
                            <div style={{fontWeight:600}}>{window.fmtTime(b.from)}-{window.fmtTime(b.to)}</div>
