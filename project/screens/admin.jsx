      ...
      <div className="card card-pad" style={{marginBottom:14}}>
        <div style={{display:'flex', alignItems:'center', gap:14, flexWrap:'wrap'}}>
          ...
          <div style={{marginLeft:'auto', display:'flex', gap:8, alignItems:'center'}}>
            ...
            <button className="btn ghost">{window.I.export} Export</button>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div className="veh-ico lg" style={{background:'var(--pea-purple-50)', color:'var(--pea-purple)', width:54, height:54}}>
        {vehicle && <window.VehicleIcon type={vehicle.type} size={36}/>}
      </div>
      <div style={{flex:1, minWidth:0}}>
        ...
        <div style={{display:'flex', gap:16, marginTop:8, flexWrap:'wrap', fontSize:12.5, color:'var(--text-2)'}}>
          ...
          <span>{window.I.car} {vehicle?.plate.split(' ').slice(0,2).join(' ')} · {vehicle?.brand}</span>
          <span>{window.I.users} {b.passengers || 1} คน</span>
        </div>
        {b.approvedBy && <div className="text-xs" style={{marginTop:6, color:'var(--ok)'}}>{window.I.check} อนุมัติโดย {b.approvedBy}</div>}
        {b.rejectReason && <div className="text-xs" style={{marginTop:6, color:'var(--danger)'}}>{window.I.x} เหตุผลที่ไม่อนุมัติ: {b.rejectReason}</div>}
      ...
      <div style={{display:'flex', flexDirection:'column', gap:6, alignItems:'flex-end'}}>
        {b.status === "booked" && (actor.role === "manager" || actor.role === "admin") && (
          <>
            <button className="btn sm" style={{background:'var(--ok)', color:'white', borderColor:'var(--ok)'}} onClick={onApprove}>{window.I.check} อนุมัติ</button>
            <button className="btn sm danger" onClick={onReject}>{window.I.x} ไม่อนุมัติ</button>
          </>
        )}
        {(b.status === "approved" || b.status === "urgent") && (
          <button className="btn sm ghost" onClick={onPrint}>{window.I.print} ใบจอง</button>
        )}
        <button className="btn sm ghost" onClick={onSelect}>ดูรายละเอียด {window.I.arrowRight}</button>
      </div>
    </div>
  );
}
            ...
            <tbody>
              {list.map((u) => (
                <tr key={u.id} onClick={() => setViewing(u)} style={{cursor:'pointer'}}>
                  ...
                  <td className="text-sm">{window.fmtDate(u.joined)}</td>
                  <td onClick={(e) => e.stopPropagation()}>
                    {tab === "pending" ? (
                      ...
                      <div style={{display:'flex', gap:4}}>
                        <button className="btn sm ghost" onClick={() => setViewing(u)}>{window.I.search} ดู</button>
                        <button className="btn sm ghost icon" onClick={() => setEditing(u)} title="แก้ไข">{window.I.edit}</button>
                      </div>
                    )}
                  </td>
                </tr>
...
function MemberDetailModal({ user, onClose, onEdit, onApprove, onReject }) {
  return (
    <window.Modal title="ข้อมูลสมาชิก" onClose={onClose} width={520}
      footer={<>
        <button className="btn" onClick={onClose}>ปิด</button>
        {onApprove && <button className="btn sm danger" style={{padding:'8px 14px'}} onClick={onReject}>{window.I.x} ปฏิเสธ</button>}
        {onApprove && <button className="btn" style={{background:'var(--ok)', color:'white', borderColor:'var(--ok)', padding:'8px 14px'}} onClick={onApprove}>{window.I.check} อนุมัติสมาชิก</button>}
        {!onApprove && <button className="btn primary" onClick={onEdit}>{window.I.edit} แก้ไขข้อมูล</button>}
      </>}>
      <div style={{display:'flex', gap:14, padding:'4px 0 18px', borderBottom:'1px solid var(--border)', marginBottom:18}}>
        ...
        <div style={{flex:1}}>
          ...
          <div className="text-sm muted" style={{marginTop:4}}>
            ...
            สมาชิกตั้งแต่ {window.fmtDate(user.joined)}
          </div>
        </div>
      </div>
      <div style={{display:'grid', gridTemplateColumns:'130px 1fr', rowGap:10, columnGap:14, fontSize:13.5}}>
        ...
        <Field label="วันที่สมัคร" value={window.fmtDate(user.joined)}/>
...
function MemberEditModal({ user, onClose, onSave }) {
  ...
  return (
    <window.Modal title={`แก้ไขข้อมูลสมาชิก · ${user.name}`} onClose={onClose} width={580}
      footer={<>
        <button className="btn" onClick={onClose}>ยกเลิก</button>
        <button className="btn primary" onClick={() => onSave(form)}>{window.I.check} บันทึก</button>
      </>}>
      <div className="col gap-3">
            ...
            <div style={{flex:1, minWidth:0}}>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:10}}>
                <div style={{padding:'8px 10px', background:'var(--surface-2)', borderRadius:8}}>
                  ...
                  <div style={{fontFamily:'var(--font-mono)', fontWeight:600, fontSize:15}}>{window.fmtNum(c.systemMileage)} กม.</div>
                </div>
                <div style={{padding:'8px 10px', background:'var(--warn-bg)', borderRadius:8}}>
                  <div className="text-xs" style={{color:'#7a5500'}}>ผู้ใช้ระบุ</div>
                  <div style={{fontFamily:'var(--font-mono)', fontWeight:700, fontSize:15, color:'var(--warn)'}}>{window.fmtNum(c.claimedMileage)} กม.</div>
                </div>
                <div style={{padding:'8px 10px', background: c.diff > 0 ? 'var(--info-bg)' : 'var(--danger-bg)', borderRadius:8}}>
                  ...
                  <div style={{fontFamily:'var(--font-mono)', fontWeight:700, fontSize:15, color: c.diff > 0 ? 'var(--info)' : 'var(--danger)'}}>{sign}{window.fmtNum(c.diff)} กม.</div>
                </div>
              </div>
              ...
              <div style={{display:'flex', gap:10, marginTop:10, alignItems:'center'}}>
                ...
                <span className="text-xs muted">ส่งคำขอเมื่อ {window.fmtDateTime(c.requestedAt)}</span>
              </div>
              ...
              {c.approvedBy && (
                <div className="text-xs" style={{marginTop:6, color: c.status === "approved" ? 'var(--ok)' : 'var(--danger)'}}>
                  ...
                  {c.approvedBy} · {window.fmtDateTime(c.approvedAt || c.requestedAt)}
