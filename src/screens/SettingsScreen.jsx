        ...
        {showSubscribeUrl && (
          <div>
            ...
            <div className="text-xs" style={{color:'var(--warn)', marginTop:6}}>
              {I.warn} URL ส่วนตัว — ห้ามแชร์กับผู้อื่น
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div style={{
      ...
      <button
        ...
        onClick={onConnect}>
        {provider.connected ? <>{I.x} ยกเลิกเชื่อมต่อ</> : <>{I.plus} เชื่อมต่อ</>}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="col gap-3">
      <div style={{display:'flex', gap:16, alignItems:'center', padding:'16px 18px', background:'linear-gradient(135deg, var(--pea-purple-50), var(--pea-orange-50))', borderRadius:12}}>
        ...
        <button className="btn ghost">{I.edit} แก้ไขโปรไฟล์</button>
      </div>
      ...
      <button className="btn ghost" style={{justifyContent:'flex-start', textAlign:'left', padding:'14px 16px'}}>
        🔒 เปลี่ยนรหัสผ่าน
        <span style={{marginLeft:'auto', color:'var(--text-3)'}}>{I.arrowRight}</span>
      </button>
      <button className="btn ghost" style={{justifyContent:'flex-start', textAlign:'left', padding:'14px 16px'}}>
